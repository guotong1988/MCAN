...
from SentenceMatchDataStream_quora import SentenceMatchDataStream,SentenceMatchFeatureStream
from SentenceMatchModelGraph_quora import SentenceMatchModelGraph
...
def collect_vocabs(train_path, with_POS=False, with_NER=False):
    ...
    for line in infile:
        ...
        sentence1 = re.split("\\s+",items[3].lower())
...
def evaluation(sess, valid_graph, devDataStream, featuresData,outpath=None, label_vocab=None):
    ...
    for batch_index in range(devDataStream.get_num_batch()):  # for each batch
        ...
        feature_cur_batch = featuresData.get_batch(batch_index)
        total += cur_batch.batch_size
        feed_dict = valid_graph.create_feed_dict(cur_batch, feature_cur_batch,is_training=True)
        [cur_correct, probs, truth, predictions] = sess.run([valid_graph.eval_correct, valid_graph.prob,valid_graph.truth, valid_graph.predictions], feed_dict=feed_dict)
        correct += cur_correct
        for item in probs:
    ...
    all_predict = np.nan_to_num(all_predict)
    print('LogLoss: %.4f' % metrics.log_loss(all_label, all_predict))
    # print('AUC: %.4f' % metrics.roc_auc_score(all_label, all_predict))
    print('Recall: %.4f' % metrics.recall_score(all_label, all_predict_binary))
    ...
    precision, recall, thresholds = metrics.precision_recall_curve(all_label, all_predict)
    all_f1 = []
...
def train(sess, saver, train_graph, valid_graph, trainDataStream, devDataStream, options, best_path, featuresData,dev_features_data):
    best_accuracy = -1
    for epoch in range(options.max_epochs):
        ...
        for batch_index in range(num_batch):  # for each batch
            ...
            feature_cur_batch = featuresData.get_batch(batch_index)
            feed_dict = train_graph.create_feed_dict(cur_batch, feature_cur_batch, is_training=True)
            ...
            if batch_index % 100 == 0:
                ...
                # evaluation
                start_time = time.time()
                acc = evaluation(sess, valid_graph, devDataStream, dev_features_data)
                duration = time.time() - start_time
...
def main(FLAGS):
    ...
    sys.stdout.flush()
    train_features_data = SentenceMatchFeatureStream("../data/train_features.csv",options=FLAGS)
    dev_features_data = SentenceMatchFeatureStream("../data/dev_features.csv", options=FLAGS,dev=True)
    ...
    devDataStream = SentenceMatchDataStream(dev_path, word_vocab=word_vocab, char_vocab=char_vocab, label_vocab=label_vocab,
                                            isShuffle=False, isLoop=True, isSort=True, options=FLAGS,dev=True)
    ...
    with tf.Graph().as_default():
        ...
        # training
        train(sess, saver, train_graph, valid_graph, trainDataStream, devDataStream, FLAGS, best_path, train_features_data,dev_features_data)
