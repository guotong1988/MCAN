...
def get_two_list_distance(token_list1, token_list2):
    if len(token_list1)==0 or len(token_list2)==0:
        return [0]*5
    return [
        int(fuzz.ratio(token_list1, token_list2)),
        int(fuzz.partial_ratio(token_list1, token_list2))+100,
        int(fuzz.token_sort_ratio(token_list1, token_list2))+200,
        int(fuzz.token_set_ratio(token_list1, token_list2))+300,
        int(fuzz.partial_token_sort_ratio(token_list1, token_list2))+400,
    ]
...
def read_all_instances(inpath, word_vocab=None, label_vocab=None, char_vocab=None, max_sent_length=100,
    ...
    for line in infile:
        ...
        if label_vocab is not None:
            ...
            label_id = int(label)
        word_idx_1 = word_vocab.to_index_sequence_rokid(sentence1)
        word_idx_2 = word_vocab.to_index_sequence_rokid(sentence2)
        if char_vocab is not None:
            char_matrix_idx_1 = char_vocab.to_character_matrix_rokid(sentence1, max_char_per_word=max_char_per_word)
            char_matrix_idx_2 = char_vocab.to_character_matrix_rokid(sentence2, max_char_per_word=max_char_per_word)
        else:
        ...
        distances = get_two_list_distance(word_idx_1,word_idx_2)

        instances.append((label, sentence1, sentence2, label_id, word_idx_1, word_idx_2, char_matrix_idx_1, char_matrix_idx_2, cur_ID, distances))
    infile.close()
    return instances
...
class InstanceBatch(object):
    def __init__(self, instances, with_char=False):
        ...
        for (label, sentence1, sentence2, label_id, word_idx_1, word_idx_2, char_matrix_idx_1, char_matrix_idx_2, cur_ID,distances) in instances:
        ...
        self.distances_list = np.array(self.distances_list, dtype=np.int32)
