...
class SentenceMatchModelGraph(object):
    ...
    def create_placeholders(self):
        ...
        self.feature_length = tf.placeholder(tf.int32, [None])
        self.features = tf.placeholder(tf.int32, [None,self.feature_len])
    ...
    def create_feed_dict(self, cur_batch, is_training=False):
        feed_dict = {
            ...
            self.features : cur_batch.distances_list,
            self.feature_length: [self.feature_len]*cur_batch.batch_size,
        }
    ...
    def create_model_graph(self, num_classes, word_vocab=None, char_vocab=None, is_training=True, global_step=None):
        ...
        if word_vocab is not None:
            ...
            with tf.device('/cpu:0'):
                self.feature_embedding = tf.get_variable("feature_embedding", trainable=True,shape=[501,180],
                                                         initializer=tf.random_normal_initializer, dtype=tf.float32)

            feature_embedding = tf.nn.embedding_lookup(self.feature_embedding, self.features) # [batch_size, feature_len, word_dim]
        ...
        # ========Bilateral Matching=====
        (match_representation, match_dim1) = match_utils.bilateral_match_func(in_question_repres, in_passage_repres,
                                                                               self.question_lengths, self.passage_lengths,
                                                                               question_mask, mask, input_dim, is_training,scope="12",
                                                                               options=options) #[batch_size,400]

        # (match_representation2, match_dim2) = match_utils.bilateral_match_func(feature_embedding, in_passage_repres,
        #                                                                        self.feature_length,
        #                                                                        self.passage_lengths,
        #                                                                        None, mask,
        #                                                                        input_dim, is_training,scope="23",
        #                                                                        options=options)  # [batch_size,400]

        # (match_representation3, match_dim3) = match_utils.bilateral_match_func(feature_embedding, in_question_repres,
        #                                                                        self.feature_length,
        #                                                                        self.question_lengths,
        #                                                                        None, question_mask,
        #                                                                        input_dim, is_training,scope="13",
        #                                                                        options=options)  # [batch_size,400]
