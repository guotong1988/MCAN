...
def read_all_lines(file_path):
    names = ["q1","q2"]+["f"+str(i) for i in range(0,28)]
    df = pd.read_csv(file_path,sep=",",encoding="ISO-8859-1",names=names)
    return df
...
def read_all_instances(inpath, word_vocab=None, label_vocab=None, char_vocab=None, max_sent_length=100,
    ...
    for line in infile:
        ...
        sentence1 = items[3].strip()
        sentence2 = items[4].strip()
        cur_ID = "{}".format(idx)
...
class SentenceMatchFeatureStream(object):
    def __init__(self,file_path,options=None,dev=False):
        df = read_all_lines(file_path)
        self.num_instances = df.shape[0]
        if self.num_instances>1000 and dev==True:
        ...
        for batch_index, (batch_start, batch_end) in enumerate(batch_spans):
            ...
            for i in range(batch_start, batch_end):
                cur_instances.append(df.iloc[i].tolist())
            cur_batch = FeatureInstanceBatch(cur_instances)
...
class SentenceMatchDataStream(object):
    def __init__(self, inpath, word_vocab=None, char_vocab=None, label_vocab=None,
                 isShuffle=False, isLoop=False, isSort=True, options=None, dev=False):
...
class FeatureInstanceBatch(object):
    def __init__(self, instances):
        self.instances = instances
        self.batch_size = len(instances)

        self.feature_batch = []  # tf.placeholder(tf.int32, [None])

        for line in instances:
            feature_line = []
            for i in range(2,30):
                feature_line.append(float(line[i]))
            self.feature_batch.append(feature_line)
