...
#         self.word_vecs = None
    ...
    def fromText_format2(self, vec_path,voc=None,pre_word_vecs=None):
        ...
        for line in vec_file:
            ...
            word = parts[1]
            vector = np.array(list(map(to_float, parts[2].split(" "))), dtype='float32')
            self.word2id[word] = cur_index 
            self.id2word[cur_index] = word
            word_vecs[cur_index] = vector
...
#         sentence = sentence.strip().lower()
    ...
    def to_index_sequence_rokid(self, sentence):
    #         sentence = sentence.strip().lower()
        sentence = sentence.strip()
        seq = []
    ...
    def to_character_matrix_rokid(self, sentence, max_char_per_word=-1):
        sentence = sentence.strip()
        seq = []
        for word in jieba.lcut(sentence,HMM=False):
            cur_seq = []
