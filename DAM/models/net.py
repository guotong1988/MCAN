...
class Net(object):
    ...
    '''
    def __init__(self, conf, fast_debug):
        self._graph = tf.Graph()
        self._conf = conf

        if self._conf['word_emb_init'] is not None and fast_debug == False:
    ...
    def build_graph(self):
        with self._graph.as_default():
            ...
            #define operations
            #response part
            H_response = tf.nn.embedding_lookup(self._word_embedding, self.response)

            if self._conf['is_positional'] and self._conf['stack_num'] > 0:
                with tf.variable_scope('positional'):
                    H_response = op.positional_encoding_vector(H_response, max_timescale=10)
            H_response_stack = [H_response]

            for index in range(self._conf['stack_num']):
                with tf.variable_scope('self_stack_' + str(index)):
                    H_response = layers.block(
                        H_response, H_response, H_response,
                        Q_lengths=self.response_len, K_lengths=self.response_len)
                    H_response_stack.append(H_response)
            ...
            for turn_t, t_turn_length in zip(list_turn_t, list_turn_length):
                ...
                for index in range(self._conf['stack_num']+1):
                    ...
                    with tf.variable_scope('t_attend_r_' + str(index)):
                        try:
                            t_a_r = layers.block(
                                Hu_stack[index], H_response_stack[index], H_response_stack[index],
                    ...
                    with tf.variable_scope('r_attend_t_' + str(index)):
                        try:
                            r_a_t = layers.block(
                                H_response_stack[index], Hu_stack[index], Hu_stack[index],
                ...
                r_a_t_stack.extend(H_response_stack)
                ...
                with tf.variable_scope('similarity'):
                    ...
                    sim = tf.einsum('biks,bjks->bijs', t_a_r, r_a_t) / tf.sqrt(200.0) # 是一个类似attention matrix的东西

                sim_turns.append(sim)


            #cnn and aggregation
            sim = tf.stack(sim_turns, axis=1) # [batch_size, max_turn_num, max_turn_len, max_turn_len, 2*(stack_num+1)]
