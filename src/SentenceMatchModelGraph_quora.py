...
class SentenceMatchModelGraph(object):
    ...
    def create_placeholders(self):
        ...
        self.features = tf.placeholder(tf.float32, [None, self.feature_len])
    ...
    def create_feed_dict(self, cur_batch, feature_cur_batch, is_training=False):
        feed_dict = {
            ...
            self.features: np.array(feature_cur_batch.feature_batch,dtype=np.float),
        }
    ...
    def create_model_graph(self, num_classes, word_vocab=None, char_vocab=None, is_training=True, global_step=None):
        ...
        features = tf.layers.batch_normalization(self.features)
        temp = tf.layers.dense(features,100,activation=tf.nn.relu)
        #========Prediction Layer=========
        match_dim = 4 * self.options.aggregation_lstm_dim
        w_0 = tf.get_variable("w_0", [match_dim+100, match_dim/2], dtype=tf.float32)
        ...
        temp2 = tf.concat([match_representation,temp],axis=-1)
        temp2 = tf.layers.batch_normalization(temp2)
        # if is_training: match_representation = tf.nn.dropout(match_representation, (1 - options.dropout_rate))
        logits = tf.matmul(temp2, w_0) + b_0
