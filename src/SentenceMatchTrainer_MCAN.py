...
from SentenceMatchDataStream_MCAN import SentenceMatchDataStream
from SentenceMatchModelGraph_MCAN import SentenceMatchModelGraph
...
def collect_vocabs(train_path, with_POS=False, with_NER=False):
    ...
    for line in infile:
        ...
        items = re.split("\t", line)
        label = str.replace(items[2],"\n","")
        sentence1 = jieba.lcut(items[0].lower(),HMM=False)
        sentence2 = jieba.lcut(items[1].lower(),HMM=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config_path', default="../configs/rokid.config", type=str, help='Configuration file.')
